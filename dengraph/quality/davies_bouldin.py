...
def davies_bouldin_score(clusters, graph):
    """
    The Davies-Bouldin score averages over all pairs of clusters. Therefore for each
    cluster, all different clusters are taken to first determine their sum of average
    distances of all instances within the two clusters to their respective centroid.
    This is normalised over the distance between the given centroids. For each pair,
    the maximum is determined.

    The target value for the Davies-Bouldin score is small, since this corresponds to
    dense but well separated clusters.
