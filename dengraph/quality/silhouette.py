...
def silhouette_score(clusters, graph):
    """
    The silhouette score is a measure how similar points within a cluster are compared
    to points in other clusters. The score considers each point with the given clusters
    and determines their average distance within their cluster as well as the minimum
    average distance to the other clusters. The value is normalised with the maximum of
    both values.

    The score itself reaches from -1 to 1 whereas -1 means, that the clustering is poor.
    A high silhouette score of one means all points are good matched to their own
    clusters.
...
def avg_inter_cluster_distance(sample, cluster, graph):
    """
    The method returns the average distance from a given sample to all other nodes
    within the given cluster. For distance calculation the distance function provided
    by the given graph is used.
...
def avg_intra_cluster_distances(sample, clusters, graph):
    """
    The method returns a list of average cluster distances to all clusters the given
    sample does not belong to. For distance calculation the distance function provided
    by the given graph is used.

    :param sample: Sample from which the distance calculations to other clusters are
        performed to.
    :param clusters: The clusters to calculate the distances for, the one with sample
        is excluded
    :param graph: The underlying graph that offers a distance function
